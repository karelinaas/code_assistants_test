...
class ReferralStatsListAPIView(ListAPIView):
    ...
    # Правка 1 из 8: потерял permission_classes
    permission_classes = [IsAuthenticated]

    def get_queryset(self):
        # Правка 2 из 8: зачем-то искал пользователя еще раз по е-мейлу, хотя он уже есть в self.request
        ...
        return ReferralStat.objects.filter(affiliate=self.request.user).annotate(
            # Правка 6 из 8: неправильная логика подсчёта, добавляю сюда свой способ вычисления
